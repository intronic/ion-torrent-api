...
(defn uri-to-edn-file
  [uri]
  (str "test/data" (str/replace uri #"/$" "") ".edn"))

(expect "test/data/my/path/to/file.edn" (uri-to-edn-file "/my/path/to/file/"))
(expect "test/data/my/path/to/file.edn" (uri-to-edn-file "/my/path/to/file"))
...
(expect {:status 200}
        (in (with-fake-routes-in-isolation
              {#".*/rundb/api/v1/.*" (fn [{uri :uri :as req}]
                                       {:status 200 :headers {} :body (pr-str (slurp (uri-to-edn-file uri)))})}
